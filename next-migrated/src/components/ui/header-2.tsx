...
export function Header({ activePage = 'home' }: HeaderProps) {
	...
	const links = [
		{ label: 'Who we are', id: 'who-we-are', href: '/who-we-are' },
							...
							<Link 
								...
								className={buttonVariants({ 
									...
									className: cn(
										...
										(activePage === link.id || pathname === link.href) && 'text-white bg-white/10'
									) 
								})} 
							>
								{link.label}
						...
						{links.map((link) => (
							<Link
								...
								className={cn(
									...
									(activePage === link.id || pathname === link.href) && "text-indigo-400"
								)}
							>
								{link.label}
							</Link>
